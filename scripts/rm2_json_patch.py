...
class RM2Converter(Downloader):
    ...
    def create_output(self, input_json, verbose=False):
        ...
        for event in input_json:
            ...
            output_json_record[self.config.RAW_PLAYER_EXTERNAL_ID] = event["playerExternalId"]
            output_json_record[self.config.RAW_PLAYER_CUSTOM_DATA] = event["playerCustomData"]
            output_json.append(output_json_record)

        return output_json

    def convert_rm2_json_to_csv(self, verbose=True):
        self.convert_rm2_to_rm1_json()
        output_json = self.create_output(self.rm1_format, verbose)
...
# continue pipeline
parsed_data = Parser.from_file(raw_data_filename, config=rm2_config).parse()
postparsed_data = PostParser(parsed_data, config=rm2_config).postparse()
feature_extractor = FeatureExtractor(postparsed_data, config=rm2_config)
